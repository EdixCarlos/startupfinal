...
const SubPage = () => {
  ...
  const handleSubJoin = async () => {
    try {
      ...
      let message = isSubscribed
        ? `Dessuscrito de ${subredditName}`
        : `Suscribete a ${subredditName}!`;
      dispatch(notify(message, 'Satisfactorio'));
    } catch (err) {
  ...
  const handleEditDescription = async () => {
    try {
      ...
      setEditOpen(false);
      dispatch(
        notify(`Actualiza las descripcion : ${subredditName}`, 'satisfactorio')
      );
    } catch (err) {
      dispatch(notify(getErrorMsg(err), 'error'));
    }
  ...
  return (
    <Container disableGutters>
      <Paper variant="outlined" className={classes.mainPaper}>
        <Paper variant="outlined" className={classes.subInfoWrapper}>
          <div className={classes.firstPanel}>
            ...
            <Typography
              ...
              color="secondary"
            >
              <CakeIcon style={{ marginRight: 5 }} /> Creado
              {' ' +
          ...
          <div className={classes.secondPanel}>
            {user && (
              ...
              >
                {isSubscribed ? 'Subscrito' : 'Suscribir'}
              </Button>
            )}
            <Typography
              variant="body2"
        ...
        {postsLoading ? (
          <LoadingSpinner text={'Cargando posts...'} />
        ) : (
          <>
            <div>
              {subPage.posts.results.length !== 0 ? (
                ...
                <div className={classes.noPosts}>
                  ...
                  <Typography variant="h6" color="secondary">
                  Sea el primero en publicar en {subredditName}!
                  </Typography>
                </div>
              )}
            </div>
